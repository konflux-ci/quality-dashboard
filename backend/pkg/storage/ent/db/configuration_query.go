...
func (cq *ConfigurationQuery) QueryConfiguration() *TeamsQuery {
	query := (&TeamsClient{config: cq.config}).Query()
...
func (cq *ConfigurationQuery) WithConfiguration(opts ...func(*TeamsQuery)) *ConfigurationQuery {
	query := (&TeamsClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withConfiguration = query
	return cq
}
...
//		GroupBy(configuration.FieldTeamName).
//		Aggregate(db.Count()).
//		Scan(ctx, &v)
...
//
//	client.Configuration.Query().
//		Select(configuration.FieldTeamName).
//		Scan(ctx, &v)
...
func (cq *ConfigurationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Configuration, error) {
	var (
		...
		_spec       = cq.querySpec()
		loadedTypes = [1]bool{
			cq.withConfiguration != nil,
		}
	)
	if cq.withConfiguration != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withConfiguration; query != nil {
		if err := cq.loadConfiguration(ctx, query, nodes, nil,
			func(n *Configuration, e *Teams) { n.Edges.Configuration = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ConfigurationQuery) loadConfiguration(ctx context.Context, query *TeamsQuery, nodes []*Configuration, init func(*Configuration), assign func(*Configuration, *Teams)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Configuration)
	for i := range nodes {
		if nodes[i].teams_configuration == nil {
			continue
		}
		fk := *nodes[i].teams_configuration
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "teams_configuration" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
