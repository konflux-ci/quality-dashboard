...
export const ListIssues: React.FC<{ issues: any }> = ({ issues }) => {
    ...
    const getSortableRowValues = (issue: Issue): (string | number)[] => {
        const { key, summary, status, priority, labels, component, age, assignee } = issue;
        return [key, summary, status, priority, labels, component, age, assignee];
    };
            ...
            <Table aria-label="Controlling text">
                ...
                <Tbody>
                    {issuesPage.map((issue, index) => {
                        return (
                            <Tr key={index}>
                                <Td dataLabel={columnNames.key}><a href={"https://issues.redhat.com/browse/" + issue.key} target={"https://issues.redhat.com/browse/" + issue.key}>{issue.key}</a></Td>
